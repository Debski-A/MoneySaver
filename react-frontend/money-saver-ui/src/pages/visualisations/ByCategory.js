...
class ByCategory extends Component {
            ...
            <Container fluid style={{ flexWrap: "wrap" }} className="d-flex fill-height">
                <Row className="d-flex fill-height">
                    <VictoryChart 
                        ...
                        domainPadding={{ x: 10 }} >
                        <VictoryAxis tickCount={20} crossAxis={false} domain={[0, 1000]} dependentAxis />
                        <VictoryAxis tickCount={31} />
                        <VictoryBar domain={[1, 31]}labels={({ datum }) => datum.y}
                            style={{
                                data: {
                                    fill: "#c43a31",
                                    width: 15
                ...
                <Row className="d-flex fill-height">
                    <Col sm={4} lg={4} xs={4} md={4}>
                        <Row>
                            <Col  >
                                <Dropdown drop="up">
                                    <Dropdown.Toggle variant="secondary" className="w-100" size="lg">{this.props.t('type')}</Dropdown.Toggle>
                                    <Dropdown.Menu >
                                        <Dropdown.Item onClick={() => this.handleTypeOnClick(0, 'income')} >{this.props.t('income')}</Dropdown.Item>
                                        <Dropdown.Item onClick={() => this.handleTypeOnClick(1, 'outcome')} >{this.props.t('outcome')}</Dropdown.Item>
                                    </Dropdown.Menu>
                                </Dropdown>
                            </Col>
                            <Col >
                                <label className="centered-label">{this.props.t(this.state.typeDescription)}</label>
                            </Col>
                        </Row>
                    </Col>
                    <Col sm={4} lg={4} xs={4} md={4}>
                        {this.state.type !== null ? (<Row>
                            <Col  >
                                <Dropdown drop="up">
                                    <Dropdown.Toggle variant="secondary" className="w-100" size="lg">{this.props.t('category')}</Dropdown.Toggle>
                                    <Dropdown.Menu>
                                        {Object.keys(this.state.categories).map((key) => <Dropdown.Item onClick={() => this.handleCategoryOnClick(key, this.state.categories[key])} key={key} >{this.state.categories[key]}</Dropdown.Item>)}
                                    </Dropdown.Menu>
                                </Dropdown>
                            </Col>
                            <Col >
                                <label className="centered-label">{this.state.categories[this.state.category]}</label>
                            </Col>
                        </Row>) : null}
                    </Col> <Col sm={4} lg={4} xs={4} md={4}>
                        {this.state.category !== null ? (<>
                            <Row>
                                <Col>
                                    <Row>
                                        <label className="centered-label">{this.props.t('month')}</label>
                                    </Row>
                                    <Row>
                                        <DatePicker
                                ...
                                <Col>
                                    <Row>
                                        <Dropdown drop="up">
                                            <Dropdown.Toggle variant="secondary" className="w-100" size="sm">{this.state.currency}</Dropdown.Toggle>
                                            <Dropdown.Menu>
                                    ...
                                    <Row>
                                        <Button variant="secondary" size="sm" className="w-50 p-2" title="GO!" onClick={this.getData}>GO!</Button>
                                    </Row>
                                </Col>
                            </Row>
                        </>) : null}
    ...
    componentDidUpdate() {
        if (this.state.currentLanguage !== i18n.language || this.state.incomeCategories !== this.props.incomeCategories || this.state.outcomeCategories !== this.props.outcomeCategories) {
            if (this.state.type === 0) {
                this.setState({
                    currentLanguage: i18n.language,
                    categories: this.props.incomeCategories,
